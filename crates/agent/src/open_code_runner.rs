...
#[derive(Debug, Clone)]
pub struct OpenCodeCommandExecutionResult {
...
#[async_trait]
pub trait OpenCodeCommandExecutor: Send + Sync {
    async fn execute(
        &self,
        invocation: &OpenCodeInvocation,
    ) -> Result<OpenCodeCommandExecutionResult>;
}

#[derive(Debug, Default)]
pub struct TokioOpenCodeCommandExecutor;

#[async_trait]
impl OpenCodeCommandExecutor for TokioOpenCodeCommandExecutor {
    async fn execute(
        &self,
        invocation: &OpenCodeInvocation,
    ) -> Result<OpenCodeCommandExecutionResult> {
        ...
        let output = command.output().await.map_err(|error| {
            Error::Agent(AgentError::ExecutionFailed(format!(
                "failed to launch opencode CLI '{}': {}",
                invocation.program, error
            )))
        })?;

        Ok(OpenCodeCommandExecutionResult {
...
pub struct OpenCodeCliRunner {
    executor: Arc<dyn OpenCodeCommandExecutor>,
}

impl Default for OpenCodeCliRunner {
    fn default() -> Self {
        Self::new(Arc::new(TokioOpenCodeCommandExecutor))
    }
}

impl OpenCodeCliRunner {
    pub fn new(executor: Arc<dyn OpenCodeCommandExecutor>) -> Self {
        Self { executor }
    }

    pub async fn run(&self, request: &OpenCodeRunRequest) -> Result<OpenCodeRunResult> {
        let invocation = build_open_code_invocation(request)?;
        ...
        parse_open_code_output(&output.stdout)
    }
}

pub fn build_open_code_prompt(
    agent_name: &str,
    role: &str,
    title: &str,
    capabilities: &str,
...
pub fn build_open_code_invocation(request: &OpenCodeRunRequest) -> Result<OpenCodeInvocation> {
...
fn parse_open_code_output(stdout: &str) -> Result<OpenCodeRunResult> {
    if stdout.trim().is_empty() {
        return Ok(OpenCodeRunResult::default());
    }

    match parse_open_code_jsonl(stdout) {
        Ok(result) => Ok(result),
        Err(ParseOutputError::InvalidJsonLine) => Ok(OpenCodeRunResult {
...
fn parse_open_code_jsonl(stdout: &str) -> std::result::Result<OpenCodeRunResult, ParseOutputError> {
...
mod tests {
    ...
    fn build_invocation_maps_provider_config() {
        ...
        let invocation = build_open_code_invocation(&request).unwrap();
    ...
    fn parse_jsonl_collects_final_message() {
        ...
        let result = parse_open_code_output(stdout).unwrap();
    ...
    fn parse_output_falls_back_to_plain_text() {
        let result = parse_open_code_output("plain text output").unwrap();
    ...
    fn parse_jsonl_surfaces_error_events() {
        ...
        let error = parse_open_code_output(stdout).unwrap_err();
